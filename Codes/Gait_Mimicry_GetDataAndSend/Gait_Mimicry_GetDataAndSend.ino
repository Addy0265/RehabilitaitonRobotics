...
double send_theta_z[4][2] = {{0}, {0}, {0}, {0}};
...
void sendData()
{  
  Serial3.write(255);   // Data Frame START
  //Serial3.print("    ");
  int i, j;
  for(i=1; i>=0; i--)
  {
    for(j=0; j<4; j++)
    {
      send_theta_z[j][i] = (int)(normalized_theta_z[j][i]);
      
      if(i == 1)
      send_theta_z[j][i] = 90 + send_theta_z[j][i];
      else if(i == 0)
      send_theta_z[j][i] = 90 - send_theta_z[j][i];
      
      send_theta_z[j][i] = constrain(send_theta_z[j][i], 0, 180);
      
      Serial3.write((int)send_theta_z[j][i]);
     //Serial3.print("  ");
    }
    //Serial3.print("  "); 
  }
  //Serial3.print("  ");  
  Serial3.write(254);   // Data Frame END
  //Serial3.print("\n"); 
}
