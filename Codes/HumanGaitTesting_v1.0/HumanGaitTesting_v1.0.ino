...
int16_t AcX[2], AcY[2], AcZ[2], Tmp[2], GyX[2], GyY[2], GyZ[2], gyro_x[2], gyro_y[2], gyro_z[2];
const int S0 = 35, S1 = 37;

double ratio[2], theta_y[2], theta_x[2], theta_z[2], ang_x[2], ang_y[2], ang_z[2];
double theta_y_old[2] = {0}, theta_x_old[2] = {0}, theta_z_old[2] = {0};
double normalized_theta_x[2] = {0}, normalized_theta_y[2] = {0}, normalized_theta_z[2] = {0};
...
void getMPUdata()
{
  ...
  */
    ...
    ang_z[i] = atan2((double)AcY[i], (double)AcX[i])*180/PI_APP;
    
    gyro_x[i] = GyX[i]/G_SENS;
    gyro_y[i] = -GyY[i]/G_SENS;
    gyro_z[i] = GyZ[i]/G_SENS;
    ...
    normalized_theta_x[i] = theta_x[i];
    
    theta_z[i] =  0.85*(theta_z_old[i] + gyro_z[i]*dt) + 0.15*ang_z[i];
    theta_z_old[i] = theta_z[i];
    normalized_theta_z[i] = theta_z[i];
...
void printData()
{
  ...
  Serial.print(" Theta_Z1 = "); Serial.print(normalized_theta_z[0]); 
  Serial.print(" | Theta_Z2 = "); Serial.println(normalized_theta_z[1]);
}
